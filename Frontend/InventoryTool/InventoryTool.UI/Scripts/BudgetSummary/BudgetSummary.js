var budgetsummaryjs = {
    init: function () {
        var obj = budgetsummaryjs;
        obj.LoadBudgetSummary();
    ...
    LoadBudgetSummary: function () {
        //initial load the Datatable setting.
        budgetsummaryjs.ReinitiateDataTable('tblbudgetList');
        $("#UserId").hide();
        $('#BudgetLines').collapse("show");

       budgetsummaryjs.UserAliasOnChangeHandler();
       var Group = document.getElementById("ddlGroup").options[1].text;
        budgetsummaryjs.StatusDropDown();
        budgetsummaryjs.FilterSummary(Group);
        //  budgetsummaryjs.GetBudgetSummary();
        ...
        $('#ddlGroup').on('change', function () {
            ...
            params.beforeSend = budgetsummaryjs.TeambeforeSend;
            params.complete = budgetsummaryjs.TeamComplete;
            params.success = budgetsummaryjs.TeamSuccess
        ...
        $('#ddlTeam').on('change', function () {
            ...
            params.beforeSend = budgetsummaryjs.ProjectbeforeSend;
            params.complete = budgetsummaryjs.ProjectComplete;
            params.success = budgetsummaryjs.ProjectSuccess
        ...
        $('#ddlProject').on('change', function () {
            ...
            params.beforeSend = budgetsummaryjs.SubProjectbeforeSend;
            params.complete = budgetsummaryjs.SubProjectComplete;
            params.success = budgetsummaryjs.SubProjectSuccess
        ...
        $("#gridview").on("click", ".clsDeletebudgetsummary", function () {
            ...
            if (promptdelete) {
                ...
                urlPath = "/BudgetSummary/DeleteBudgetRequest";
                if (BudgetRequest_Id != '') {
                    ajaxRequest = $.ajax({
                        type: "Get",
                        cache: false,
                        ...
                        success: function (result) {
                            if (result.operationstatuscode == 3) {
                                ...
                                budgetsummaryjs.fn_ReloadSummaryGrid();
                            }
                            else {
        ...
        $("#ddlCategoryApprove").on("change", function () {
            ...
            row = $("#gridview").find("#tblbudgetList").find("tr.selected");
            data = $(row);
            selected = $(this).find("option:selected").val();
            BudgetRequest_Id = $(data).attr("data-BudgetRequest_Id");
            Approval_Level_Id = $("#ddlApprovalLevels").find("option:selected").val();
            budgetsummaryjs.fn_TotalOriginalAsk(BudgetRequest_Id, selected, Approval_Level_Id);
           ...
           var row, data, BudgetRequest_Id, Approval_Level_Id,
            ...
            if (Approval_Level_Id > 0) {
                ...
                if (Approval_Level_Id == 5) {
                    ...
                    $("#divAmount").show();
                    $('#HideOR').show();
                    budgetsummaryjs.Fn_BulkApprovalAmountCalculation();
                }
                budgetsummaryjs.fn_TotalOriginalAsk(BudgetRequest_Id, Category_Id, Approval_Level_Id);
        ...
        $('#btnSumapprovalsubmit').on('click', function () {
            ...
            if (promptbulksubmitapproval) {
                urlPath = "/BudgetSummary/BudgetBulkApproval";
                if (true) {
                     $.ajax({
                        type: "POST",
                        cache: false,
                        ...
                        success: function () {
                            ...
                            budgetsummaryjs.fn_RefreshApprovalPanel();
                            //now reload the summary Grid.
                            //$("#btnFindBudgetSummary").trigger("click");
                            budgetsummaryjs.FilterSummary($("#ddlGroup").find("option:selected").text());
                            //hide the Approval panel
                            $("#ApprovalWorkFlow").hide();
    ...
    GetBudgetSummary: function () {
        ...
        urlPath = "/BudgetSummary/GetBudgetSummary";
        ...
        ajaxRequest = $.ajax({
            ...
            success: function (result) {
                ...
                if (result.SubmittedAmount!=null) {
                    $("#txtTotalSubmitted").val(budgetsummaryjs.addCommas((result.SubmittedAmount).toFixed(0)));
                }
                if (result.ManagerAmount != null) {
                    $("#txtTotalManagerApproved").val(budgetsummaryjs.addCommas((result.ManagerAmount).toFixed(0)));
                }
                if (result.DirectorAmount != null) {
                    $("#txtTotalDirectorApproved").val(budgetsummaryjs.addCommas((result.DirectorAmount).toFixed(0)));
                }
                if (result.SLTAmount != null) {
                    $("#txtTotalSLTApproved").val(budgetsummaryjs.addCommas((result.SLTAmount).toFixed(0)));
                }
    ...
    FilterSummary: function (Groups) {
        var  Group, Team, Project,TextSearch, SubProject,  Category, Class, InvestmentType, Priority, Status;
    ...
    StatusDropDown: function () {
        ...
        urlPath = "/Common/GetApprovalStatus";
        ajaxRequest = $.ajax({
            type: "Post",
            cache: false,
            url: urlPath,
            ...
            success: function (result) {
                ...
                $('.clsBulkStatus').empty();
                listItems = "<option value='0'>--Select Status--</option>";
                if (result.length > 0) {
                    for (i = 0; i < result.length; i++) {
    ...
    fn_OnSelectedRowHandler: function (obj) {
        ...
        if (Approval_Status_Id != 1) {
            ...
            budgetsummaryjs.fn_RefreshApprovalPanel();
            budgetsummaryjs.fn_GetOriginalAsk();
        }
        else {
            //hide the Approval panel
            $("#ApprovalWorkFlow").hide();
...
//populating SLT approved amount
    fn_PopulatingSLT: function (obj)
    {      
        var data = $(obj);
        $(obj).addClass("selected").siblings().removeClass("selected");
        //Approval_Status_Id = $(data).attr("data-Approval_Status_Id");       
        ...
        if (SLTAmount != 0)
        {
            $("#txtTotalSLTApproved").val(budgetsummaryjs.addCommas((SLTAmount).toFixed(0)));
        }
        else
        {
    ...
    fn_GetOriginalAsk: function ()
    {
        ...
        var row = $("#gridview").find("#tblbudgetList").find("tr.selected");
       var data = $(row);
        ...
        $("#Reduction").val(0);
        if (Ask_Amount != null) {
            $("#ApprovedAmountSummary").val(Number(Ask_Amount).toFixed(0));
        }
        $("#leveltext").val(Line_Item);
        $("#divAmount").hide();
        budgetsummaryjs.fn_TotalOriginalAsk(BudgetRequest_Id, Category_Id, 1);
    ...
    fn_TotalOriginalAsk: function (BudgetRequest_Id, Category_Id, Approval_Level_Id) {
        var  Approver_Reduction_Percentage;
        Approver_Reduction_Percentage = $("#Reduction").val().trim();
        urlPath = "/BudgetSummary/GetBulkAmount";
        if (true) {
             $.ajax({
                type: "Get",
                cache: false,
                ...
                success: function (result) {
                    ...
                    {
                        $("#Originalask").val((result[0].TotalOrignalAsk).toFixed(0));
                        
                    }
                    //if (Approver_Reduction_Percentage!=0)
                    //{
                   if (result[0].TotalApproved != null) {
                       $("#Approvedamount").val((result[0].TotalApproved).toFixed(0));
                    }
                    //}
                    //else {
                    //    $("#Approvedamount").val("");
  ...
  //This function refresh the grid data.
    fn_ReloadSummaryGrid: function () {
        var urlPath = "/BudgetSummary/GetBudgetSummary";
        if (true) {
             $.ajax({
                type: "Get",
                cache: false,
    ...
    {
       // budgetsummaryjs.Fn_GettingSubProject(-1);
    ...
    SubProjectSuccess: function (result)
    {
        ...
        listItems = "<option value='0'>--Select Sub Project--</option>";
        for (i = 0; i < result.length; i++) {
            listItems += "<option value='" +
    ...
    Fn_BulkApprovalAmountCalculation: function()
    {
        ...
        if (Approver_Reduction_Percentage >= 0 && Approver_Reduction_Percentage <= 100)
        {
            ...
            $("#ApprovedAmountSummary").val((SubmittedApprovedAmount.toFixed(0)));
        }
        else
        {
            $("#ApprovedAmountSummary").val('');
    ...
    fn_ExportToExcel: function () {
        ...
        var Group_Id, Team_Id, Project_Id,TextSearch, Sub_project_Id, BudgetRequest_Id, Category_Id, Class_Id, InvestmentType_Id, Priority_Id, Approval_Status_Id;
        Approval_Status_Id = $("#ddlApprovalStatus").find("option:selected").val();
        Category_Id = $("#ddlCategory").find("option:selected").val();
        Group_Id = $("#ddlGroup").find("option:selected").val();
        Team_Id = $("#ddlTeam").find("option:selected").val();
        ...
        window.open("/BudgetSummary/ExportToExcel?BudgetRequest_Id=" + BudgetRequest_Id +
            "&Category_Id=" + Category_Id +
                "&Group_Id=" + Group_Id +
                "&Team_Id=" + Team_Id +
                "&Project_Id=" + Project_Id +
